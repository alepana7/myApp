...
class GetByNameTask : AsyncTask<String?, Void?, Location?>() {
    protected override fun doInBackground(vararg p0: String?): Location? {
        var loc: Location? = null
        try {
            val url =
                URL("https://api.openweathermap.org/data/2.5/weather?q=" + p0[0] + "&units=metric&lang=it&appid=" + Constants.KEY)
